...
from model.transformer_model.roberta.RobertaForSequenceClassification import RobertaForSequenceClassificationFeatures
...
class ClassificationModel:
    def __init__(
        ...
        if self.args.onnx:
            ...
            if not self.args.quantized_model:
                if args['value_head'] == 0:
                    self.model = model_class.from_pretrained(
                        model_name, config=self.config, **kwargs
                    )
                else:
                    self.model = model_class.from_pretrained(
                        model_name, config=self.config, value_head=args['value_head'], **kwargs
                    )
    ...
    def train_model(
        ...
        train_dataloader = DataLoader(
            train_dataset,
            # Added to have the data reshuffled at every epoch
    ...
    def compute_metrics(
        ...
        for metric, func in kwargs.items():
            if metric.startswith("prob_"):
                ...
                extra_metrics[metric] = func(labels, preds, average='macro')
            else:
        ...
        if not multi_label and self.args.onnx:
            ...
            if isinstance(to_predict[0], list):
                if self.args.model_type in ["layoutlm", "layoutlmv2"]:
                    ...
                    eval_examples = (list(zip(*to_predict)))
                    # eval_examples = (
                    #     *zip(*to_predict),
